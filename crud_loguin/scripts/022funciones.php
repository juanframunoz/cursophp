...
function login(){
	...
	$sql="select usuario, password from usuarios where usuario='$usuario' && password='$password'";

	$resultado= mysqli_query($conectar,$sql);

	if (!$resultado){
		die("fallo en la ejecución".mysqli_error($conectar));
	}

	$fila= mysqli_fetch_assoc($resultado);
	if(is_array($fila) && count($fila)>0){
		header("location:022_3listar.php"); //redirecciona a una url
		exit();
	}
